...
import ch.internettechnik.anouman.backend.session.deltaspike.jpa.repository.EditorTestRepository;
...
@Stateless
public class EditorTestFacade {
    @PersistenceContext
    public EntityManager em;
    @Inject
    EditorTestRepository repo;
    ...
    */
    ...
    /**
     * A QueryDSL example of providing API for EditorTestFacade users where they can just
    ...
    public List<EditorTest> findEditorTest(String filter) {
        if (filter != null && !filter.isEmpty()) {
            return repo.findByErsterLikeIgnoreCase(filter + "%");
        }
        return repo.findAll();
    }
